  ...
  export default {
    ...
    async fetchData() {
      ...
      try {
        ...
        // Filter out bookings where 'flight' field is null to include only waitlisted passengers
        const waitlistBookings = bookingResponse.data.filter(booking => booking.flight === null);

        this.bookings = waitlistBookings.map(booking => ({
          ...booking,
